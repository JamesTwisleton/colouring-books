    ...
    <div className="flex flex-col gap-3">
      <div
        className={`relative border-2 border-dashed rounded-2xl transition-colors ${
          dragging ? "border-[#ff6b6b] bg-orange-50" : "border-gray-200 hover:border-gray-300"
        } ${currentUrl ? "p-2" : "p-8"}`}
        onDragOver={(e) => {
          e.preventDefault();
          setDragging(true);
      ...
      >
        {currentUrl ? (
          ...
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-gray-700">Drop your illustration here</p>
              <p className="text-xs text-gray-400 mt-0.5">PNG, SVG, JPEG or WebP — max 10 MB</p>
            </div>
            <button
              onClick={() => inputRef.current?.click()}
      ...
      <p className="text-xs text-gray-400 text-center leading-relaxed">
        Tip: use a PNG with a white or transparent background so the child colours between the lines.
      </p>
